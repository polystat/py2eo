...
class GenericTests(BaseTestCase):
    ...
    def test_generic_hashes(self):
        ...
        if ((sys.version_info[slice(2, (2) + 1, 1)] > (3, 2) )):
    ...
    def test_pickle(self):
        ...
        for proto in range((pickle.HIGHEST_PROTOCOL + 1)):
            ...
            pass
        samples = [Any, Union, Tuple, Callable, ClassVar, Union[int, str], ClassVar[List], Tuple[int, ], Callable[[str,], bytes], typing.DefaultDict, typing.FrozenSet[int]]
        for s in samples:
            for proto in range((pickle.HIGHEST_PROTOCOL + 1)):
    ...
    def test_copy_and_deepcopy(self):
        ...
        things = [Union[T, int], Tuple[T, int], Callable[, T], Callable[[int,], int], Tuple[Any, Any], Node[T], Node[int], Node[Any], typing.Iterable[T], typing.Iterable[Any], typing.Iterable[int], typing.Dict[int, str], typing.Dict[T, Any], ClassVar[int], ClassVar[List[T]], Tuple['T', 'T'], Union['T', int], List['T'], typing.Mapping['T', int]]
        for t in (things + [Any,]):
    ...
    def test_weakref_all(self):
        T = TypeVar('T')
        things = [Any, Union[T, int], Callable[, T], Tuple[Any, Any], Optional[List[int]], typing.Mapping[int, str], typing.Match[bytes], typing.Iterable['whatever']]
        for t in things:
...
class ForwardRefTests(BaseTestCase):
    def test_basics(self):
        class Node(Generic[T]):
            ...
            def add_both(self, left, right = None, stuff = None, blah = None):
                self.left = left
                self.right = right
...
try:
    ...
    pass
from test import ann_module as ann_module
from test import ann_module2 as ann_module2
from test import ann_module3 as ann_module3
from test import ann_module5 as ann_module5
from test import ann_module6 as ann_module6
...
class NamedTupleTests(BaseTestCase):
    ...
    def test_namedtuple_pyversion(self):
        if ((sys.version_info[slice(2, (2) + 1, 1)] < (3, 6) )):
            with self.assertRaises(TypeError):
...
class IOTests(BaseTestCase):
    ...
    def test_io_submodule(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            from typing.io import IO as IO
            from typing.io import TextIO as TextIO
            from typing.io import BinaryIO as BinaryIO
            from typing.io import __all__ as __all__
            from typing.io import __name__ as __name__
...
class RETests(BaseTestCase):
    ...
    def test_re_submodule(self):
        with warnings.catch_warnings(record=True) as w:
            warnings.filterwarnings("default", category=DeprecationWarning)
            from typing.re import Match as Match
            from typing.re import Pattern as Pattern
            from typing.re import __all__ as __all__
            from typing.re import __name__ as __name__
...
class AllTests(BaseTestCase):
    ...
    def test_all_exported_names(self):
        ...
        computed_all = {k for (k, v) in vars(typing).items() if ((k in actual_all ) or (not k.startswith('_') and ((k not in {'io', 're'} ) and (not k.endswith(('Meta', '_contra', '_co')) and (not (k.upper() == k ) and (getattr(v, '__module__', None) == typing.__name__ ))))))}
