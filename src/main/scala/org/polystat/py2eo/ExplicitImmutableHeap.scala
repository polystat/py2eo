...
//      println(s"procSt($s)")
      ...
      def ptr4ident(name : Ident) : Expression.T = {
        ...
        if (s._1 == VarScope.Local || s._1 == VarScope.Arg) Ident(name.name + "Ptr", name.ann.pos) else
          if (s._1 == VarScope.NonLocal || s._1 == VarScope.ImplicitNonLocal)
            CallIndex(isCall = false, Ident("closure", name.ann.pos),
      ...
      def procCall(c : CallIndex, ns : Names) : CallIndex = {
        ...
        CallIndex(isCall = true, callme(whom),
      ...
      s match {
        case FuncDef(name, args, None, None, None, body, Decorators(List()), vars, isAsync, ann) =>
          ...
          val (pushLocals, ns0) = locals.foldLeft((List[Statement](startHeap), ns))((acc, name) => {
            ...
            (
              acc._1 :+
              CreateConst(name._1 + "Ptr", CallIndex(isCall = true, Ident("nextFreePtr", pos), List((None, heap)), pos), pos) :+
              CreateConst(newHeap, CallIndex(isCall = true, Ident("append2heap", pos),
...
//            ))))
          ...
          val mkFun = CreateConst(newHeap, CallIndex(isCall = true, Ident("immArrChangeValue", ann.pos),
...
//        case Assign(List(CallIndex(_, _, _))) | Assign(List(_, CallIndex(_, _, _))) => ???
        ...
        case Assign(List(Ident(_, _), CallIndex(true, pi@Ident("print", _), args, annc)), ann) =>
          (Assign(List(CallIndex(isCall = true, pi, args.map(x => (x._1, procIdentInRhs(x._2))), annc.pos)), ann.pos), ns, false)
...
//            Assign(List(whom1, whom)),
            ...
            CreateConst(newHeap, CallIndex(isCall = true, Ident("immArrChangeValue", ann.pos),
