...
object ExplicitMutableHeap {
  ...
  def valueGet(e: Expression.T): CallIndex =
    CallIndex(isCall = true, Field(valueHeapId(e.ann), "get", e.ann.pos), List((None, e)), e.ann.pos)
  def ptrGet(e: Expression.T): CallIndex =
    CallIndex(isCall = true, Field(ptrHeapId(e.ann), "get", e.ann.pos), List((None, e)), e.ann.pos)
  def index(arr: Expression.T, ind: Expression.T): CallIndex =
    CallIndex(isCall = false, arr, List((None, ind)), ind.ann.pos)

  private def newValue(e : Expression.T) =
    CallIndex(isCall = true, Field(valueHeapId(e.ann), "new", e.ann.pos), List((None, e)), e.ann.pos)
  def newPtr(e : Expression.T): CallIndex =
    CallIndex(isCall = true, Field(ptrHeapId(e.ann), "new", e.ann.pos), List((None, e)), e.ann.pos)
  ...
  def explicitHeap(st: Statement, ns: Names): (Suite, Names) = {
    def procSt(closure : String => Int, scope : String => (VarScope.T, GeneralAnnotation),
        ...
        val (Left(result), _) = procExpr(
          (lhs, e, ns) => {
            e match {
              ...
              case Ident(name, ann)  =>
                val e1 = if (
                  scope(name)._1 != VarScope.Arg && scope(name)._1 != VarScope.Local &&
                  !List(valueHeap, ptrHeap, allFunsName, "closure").contains(name)
                ) {
                  index(Ident("closure", ann.pos), IntLiteral(closure(name), ann.pos))
                } else {
                  e
                }
              ...
              case CallIndex(true, whom, args, ann) => // todo: this implementation is incorrect, it evals whom twice! we must extract whom to a variable!
                val args2 = args.map {
                  case x@(
                    (None, NoneLiteral(_)) | (None, BoolLiteral(_, _))
                    | (None, IntLiteral(_, _)) | (None, StringLiteral(_, _))
                    ) =>
                    (None, newValue(x._2))
                  case x@(None, _) => x
                }
                (
                  Left(
                    CallIndex(
                      isCall = true, index(allFunsId(ann), index(whom, IntLiteral(0, ann.pos))),
                      (None, whom) :: args2, ann.pos
                    )
                  ),
                  ns
                )
      ...
      st match {
        case FuncDef(name, args, None, None, None, body, Decorators(List()), vars, isAsync, ann) =>
          ...
          val (closure, _) = add2closure.foldLeft((HashMap[String, Int](), 1))(
            (acc, name) => (acc._1.+((name._1, acc._2)), acc._2 + 1)
          )
          ...
          val f1 = FuncDef(
            ...
            None, None, None, Suite(createLocals.toList :+ body1, body1.ann.pos), Decorators(List()), HashMap(),
            isAsync, ann.pos
          )
          val fs2 = fs1 :+ f1
          val rhs = CollectionCons(
            ...
            Suite(
              List(
                ...
                Assign(
                  List(
                    CallIndex(
                      isCall = true, Field(ptrHeapId(ann), "set", ann.pos),
                      List((None, Ident(name, ann.pos)), (None, newValue(Ident("nextClosure", ann.pos)))), ann.pos
                    )
                  ),
                  ann.pos
                )
        ...
        case Assign(List(lhs, rhs), ann) => (
          Assign(
            List(
              CallIndex(
                isCall = true, Field(ptrHeapId(ann), "set", ann.pos),
                List((None, pe(lhs = true, lhs)), (None, newValue(pe(lhs = false, rhs)))),
                ann.pos
              )
            ),
            ann.pos
        ...
        case Suite(l, ann) =>
          val (l1, fs) = l.foldLeft((List[Statement](), functions))(
            (acc, st) => {
    ...
    val (rest, allFuns) = procSt(
      _ => ???, _ => (VarScope.Global, new GeneralAnnotation()), SimpleAnalysis.computeAccessibleIdents(st), List()
    )
    val outst = Suite(
      allFuns :+
      CreateConst(
        "allFuns",
        CollectionCons(Expression.CollectionKind.List, allFuns.map(f => Ident(f.name, f.ann.pos)), nopos), nopos
      ) :+
      rest, st.ann.pos
    )
    (outst, ns)
