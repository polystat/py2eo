...
case class Node(name: String, data: Option[String], children: List[Node] = List.empty) {

  /** Converter to an astranaut node class */
  implicit def toAstranautNode: base.Node = {
    val result = new DraftNode.Constructor
    result.setName(name)
    data.foreach(result.setData)
    result.setChildrenList(children.map(_.toAstranautNode).asJava)
